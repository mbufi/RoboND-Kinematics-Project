...
def handle_calculate_IK(req):
    ...
    if len(req.poses) < 1:
        ...
        ##RUN BEFORE THE LOOP TO SAVE TIME##
        ...
        alpha0, alpha1, alpha2, alpha3, alpha4, alpha5, alpha6 = symbols('alpha0:7') #theta joint angles
        ...
        # The link from the beginning all the way to the end
        ...
        for x in xrange(0, len(req.poses)):
            ...
            #R_EE = simplify(R_roll * R_pitch * R_yaw)
            #R_EE = (R_roll * R_pitch * R_yaw).evalf(subs={roll: roll, pitch: pitch, yaw: yaw})
            ...
            R3_6 = (R0_3) ** -1 * R_EE[0:3, 0:3] ## Extract rotational component of transform matrix

            # Formula for taking the rotation to euler
            (theta4, theta5, theta6) = tf.transformations.euler_from_matrix(np.array(R3_6[0:3, 0:3]).astype(np.float64),
                                                                            "ryzx")
